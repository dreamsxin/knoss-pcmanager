...
BOOL CWinPathApi::GetRealPath( LPCTSTR lpOldPath,CString &strRealPath)
{
	TCHAR		initBuffer[1] = {0};
	LPTSTR		lpBuffer = NULL;
	DWORD		nSize = 0;
	BOOL		bRes = FALSE;

	strRealPath = lpOldPath;
	ExpandEnvironmentStrings(strRealPath);
	if ( strRealPath[1] != _T(':') )
	{
		// 不是正规的 C:\a.txt
		nSize = ::SearchPath( NULL,strRealPath,NULL,1,initBuffer,NULL);

		if ( nSize > 1 )
		{
			TCHAR*	szNewBuffer = new TCHAR[nSize+1];
			::SearchPath(NULL,strRealPath,NULL,nSize+1,szNewBuffer,NULL);
			strRealPath = szNewBuffer;
			delete szNewBuffer;
		}
	}
	return TRUE;
