	...
	do 
	{
		...
		for(int i=0; i<m_arrDownloadItem.GetSize() && !m_bCanceled; ++i)
		{
			...
			else
			{
				...
				CString strFilename;
				if( _IsUrlAllowed(item.strURL) && GetFileNameFromUrl(item.strURL, strFilename) )
				{
					// 兼容正常文件名 
					...
					if(!IsFileExist(item.strFilename))
					{
						...
						if(!IsFileExist(item.strFilename))
						{
							...
							if ( toDownloadFromWeb && !m_bCanceled )
							{
								item.strFilename.Format(_T("%s\\%s"), m_strDownloadPath, strFilename);
							}
							
							if(toDownloadFromWeb && !IsFileExist(item.strFilename) && !m_bCanceled)
							{
								m_bDownloadHasReceivedData = FALSE;
...
{
	 _IfFileExistDelete(  _T("$hf_mig$") );
	 _IfFileExistDelete(  _T("ie7updates") );
	 _IfFileExistDelete(  _T("ie8updates") );

	 CString strKBName, strKBStr;
	 _FormatKBString( item.nID, strKBStr );
	 strKBName.Format( L"$NtUninstall%s$", strKBStr );
	 _IfFileExistDelete( strKBName );
}

void CImplRepairVul::_IfFileExistDelete( LPCTSTR lpszFileName )
{
	...
	CString strDetectPath = m_strSystemRoot + lpszFileName;
...
BOOL CImplRepairVul::_IsUrlAllowed( LPCTSTR lpszUrl )
{
	LPCTSTR szAllowedHosts[] = {
		_T(".microsoft.com"),
		_T(".kingsoft.com"),
		_T(".duba.net"),
		_T(".ijinshan.com"),
	...
	CString strHost, strPath, strFileName;
	if( SplitDownloadUrl(lpszUrl, strHost, strPath, strFileName) )
	{
		strHost.MakeLower();
		
		LPCTSTR szHost = strHost;
		INT nLenHost = strHost.GetLength();
		for(int i=0; i<SIZE_OF_ARRAY(szAllowedHosts); ++i)
		{
			LPCTSTR sz = szAllowedHosts[i];
			INT n = _tcslen(sz);
		
			if ( nLenHost>n && _tcscmp(sz, szHost + (nLenHost-n))==0 )
			{
				return TRUE;
			}		
		}
