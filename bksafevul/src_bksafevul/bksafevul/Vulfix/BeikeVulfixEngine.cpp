...
public:
	...
	virtual void OnVulfixEvent( TVulFixEventType evt, int nKbId, DWORD dwParam1, DWORD dwParam2 )
	{
		...
		*/
        ...
        {
            BSTR bstr = NULL;
            if( theEngine && theEngine->m_pRepairVul && SUCCEEDED(theEngine->m_pRepairVul->GetItemFilePath( nKbId, &bstr )))
            {
                CString strFilePath = bstr;
                SysFreeString(bstr);

                LPCTSTR pslash = _tcsrchr(strFilePath, _T('\\'));
                if(pslash)
                {
                    ++pslash;
                    CString strFilename;
                    if(_tcsnicmp(pslash, BK_FILE_PREFIX, _tcslen(BK_FILE_PREFIX))==0)
                        strFilename = pslash + _tcslen(BK_FILE_PREFIX);
                    else
                        strFilename = pslash;

                    if(evt==EVulfix_DownloadError || evt==EVulfix_DownloadEnd)
                    {
						BOOL bCanceled = dwParam1;
						BOOL bDownFromMirror = dwParam2 & ERDOWN_MIRROR_USED;
						BOOL bOfficialSiteCannotReached = dwParam2 & ERDOWN_CANNOT_REACH_OFFICIAL;
						if(evt==EVulfix_DownloadError)
						{
							// 非取消
							if(!bCanceled)
								SendNormalStatic(strFilename, _T("down_fail"), bDownFromMirror ? _T("mir") : NULL);
						}
						else // EVulfix_DownloadEnd
						{
							// 对本地文件已存在的不报告已下载成功
							if(dwParam2&ERDOWN_LOCAL_NOT_EXISTS)
								SendNormalStatic(strFilename, _T("down_ok"), bDownFromMirror ? _T("mir") : NULL);
						}
                    }
                    else if(EVulfix_Download_Check_Error==evt)
                    {
                        CString strCode;
                        strCode.Format(_T("code=%x&p=%d"), dwParam1, dwParam2);		
                        SendNormalStatic(strFilename, _T("certcheck_fall"), strCode);
                    }
                    else
                    {
                        if(EVulfix_InstallEnd==evt)
                        {
                            SendNormalStatic(strFilename, m_bSmartInstallBegined ? _T("smartinst_ok") : _T("inst_ok"));
                        }
                        else
                        {
                            BOOL bInstFail = EVulfix_InstallBeginUser==evt || !m_bSmartInstallBegined;
                            CString strCode;
                            strCode.Format(_T("code=%x&res=%x"), dwParam2, dwParam1);	// res==7 判断是否被360 拦截 
                            SendNormalStatic(strFilename, bInstFail ? _T("inst_fail") : _T("smartinst_fail"), strCode);
                        }
                    }
                }
            }
