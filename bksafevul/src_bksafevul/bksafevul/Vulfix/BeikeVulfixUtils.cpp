...
// #include <statistics/InstallStatistics.h>
#include <TlHelp32.h>
#include <vector>
#include <algorithm>
...
void SendStatic( LPCTSTR szUrl )
{
//	CStatistics::GetGlobalPtr()->Report(szUrl);	
}

void SendNormalStatic(LPCTSTR szPkgName, LPCTSTR szStaticType, LPCTSTR szOther)
{
	CString strUrl = _T("http://stat.ijinshan.com/st.html?m=vulfix");
	if(szPkgName)
	{
		CString strPkgName = szPkgName;
		strPkgName.MakeLower();
		strUrl.AppendFormat(_T("&pkg=%s"), strPkgName);
	}
	if(szStaticType)
		strUrl.AppendFormat(_T("&%s"), szStaticType);
	if(szOther)
		strUrl.AppendFormat(_T("&%s"), szOther);
	SendStatic(strUrl);
}
