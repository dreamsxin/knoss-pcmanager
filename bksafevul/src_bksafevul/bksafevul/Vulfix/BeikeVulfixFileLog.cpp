...
BOOL CBeikeVulfixFileLog::Load()
{
	CString strIniFile;
	BKSafeConfig::Get_Vulfix_IniFile( strIniFile );
	LoadIniSectionKeyValues( strIniFile, _T("downloadedfiles"), m_DownloadedFiles);
	LoadIniSectionKeyValues( strIniFile, _T("installedresults"), m_InstallResults);
...
{	
	...
	if(!CreateDirectoryNested(szPathTo))
	{
		...
		CBkSafeMsgBox2::Show(strTitle, BkString::Get(IDS_VULFIX_5010), MB_OK | MB_ICONERROR);
		return FALSE;
	}
	...
	INT id = CBkSafeMsgBox2::Show(strTitle, BkString::Get(IDS_VULFIX_5012), MB_YESNO | MB_ICONQUESTION);
	if(id==IDNO)
		return FALSE;
