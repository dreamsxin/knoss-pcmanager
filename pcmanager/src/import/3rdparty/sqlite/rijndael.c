...
// API
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
int RijndaelInit(Rijndael* ctx, EM_Mode mode,EM_Direction dir,const UINT8 * key,EM_KeyLength keyLen,UINT8 * initVector)
{
  int i;
  ...
  assert(ctx);
  // Not initialized yet
  ctx->m_state = em_Rijndael_Invalid;

  // Check the mode
  if ((mode != em_Rijndael_CBC) &&
      (mode != em_Rijndael_ECB) && 
      (mode != em_Rijndael_CFB1))
        return RIJNDAEL_UNSUPPORTED_MODE;
  ctx->m_mode = mode;

  // And the direction
  if ((dir != em_Rijndael_Encrypt) && (dir != em_Rijndael_Decrypt))
  ...
  if(initVector)
  {
    ...
    {
      ctx->m_initVector[i] = initVector[i];
    }
  } else {
    // zero init vector
    for(i = 0;i < MAX_IV_SIZE;i++)
    {
      ctx->m_initVector[i] = 0;
    }
  }
  ...
  switch(keyLen)
  {
    case em_Rijndael_Key16Bytes:
      uKeyLenInBytes = 16;
      ctx->m_uRounds = 10;
    break;
    case em_Rijndael_Key24Bytes:
      uKeyLenInBytes = 24;
      ctx->m_uRounds = 12;
    break;
    case em_Rijndael_Key32Bytes:
      uKeyLenInBytes = 32;
      ctx->m_uRounds = 14;
    break;
    default:
  ...
  for(i = 0;i < (int)uKeyLenInBytes;i++)keyMatrix[i >> 2][i & 3] = key[i]; 

  RijndaelKeySched(ctx, keyMatrix);

  if(ctx->m_direction == em_Rijndael_Decrypt)RijndaelKeyEncToDec(ctx);

  ctx->m_state = em_Rijndael_Valid;
...
int RijndaelBlockEncrypt(Rijndael* ctx, const UINT8 *input,int inputLen,UINT8 *outBuffer)
{
  ...
  if(ctx->m_state != em_Rijndael_Valid)return RIJNDAEL_NOT_INITIALIZED;
  if(ctx->m_direction != em_Rijndael_Encrypt)return RIJNDAEL_BAD_DIRECTION;
  ...
  switch(ctx->m_mode){
    case em_Rijndael_ECB: 
      for(i = numBlocks;i > 0;i--)
      {
        RijndaelEncrypt(ctx, input,outBuffer);
        input += 16;
        outBuffer += 16;
      }
    break;
    case em_Rijndael_CBC:
      ((UINT32*)block)[0] = ((UINT32*)ctx->m_initVector)[0] ^ ((UINT32*)input)[0];
      ((UINT32*)block)[1] = ((UINT32*)ctx->m_initVector)[1] ^ ((UINT32*)input)[1];
      ((UINT32*)block)[2] = ((UINT32*)ctx->m_initVector)[2] ^ ((UINT32*)input)[2];
      ((UINT32*)block)[3] = ((UINT32*)ctx->m_initVector)[3] ^ ((UINT32*)input)[3];
      RijndaelEncrypt(ctx, block,outBuffer);
      input += 16;
    ...
    break;
    case em_Rijndael_CFB1:
#if STRICT_ALIGN 
      memcpy(iv,ctx->m_initVector,16); 
#else  /* !STRICT_ALIGN */
      *((UINT32*)iv[0]) = *((UINT32*)(ctx->m_initVector   ));
      *((UINT32*)iv[1]) = *((UINT32*)(ctx->m_initVector + 4));
      *((UINT32*)iv[2]) = *((UINT32*)(ctx->m_initVector + 8));
      *((UINT32*)iv[3]) = *((UINT32*)(ctx->m_initVector +12));
#endif /* ?STRICT_ALIGN */
      for(i = numBlocks; i > 0; i--)
      {
        for(k = 0; k < 128; k++)
        {
          ...
          *((UINT32*)(block+12)) = *((UINT32*)iv[3]);
          RijndaelEncrypt(ctx, block,block);
...
int RijndaelPadEncrypt(Rijndael* ctx, const UINT8 *input, int inputOctets, UINT8 *outBuffer)
{
  ...
  if(ctx->m_state != em_Rijndael_Valid)return RIJNDAEL_NOT_INITIALIZED;
  if(ctx->m_direction != em_Rijndael_Encrypt)return RIJNDAEL_NOT_INITIALIZED;
...
int RijndaelBlockDecrypt(Rijndael* ctx, const UINT8 *input, int inputLen, UINT8 *outBuffer)
{
  ...
  if (ctx->m_state != em_Rijndael_Valid)
      return RIJNDAEL_NOT_INITIALIZED;

  if ((ctx->m_mode != em_Rijndael_CFB1) &&
      (ctx->m_direction == em_Rijndael_Encrypt))
  ...
  switch(ctx->m_mode)
  {
    case em_Rijndael_ECB: 
      for (i = numBlocks; i > 0; i--)
      {
        RijndaelDecrypt(ctx, input,outBuffer);
        input += 16;
        outBuffer += 16;
      }
    break;
    case em_Rijndael_CBC:
      ...
      *((UINT32*)iv[3]) = *((UINT32*)(ctx->m_initVector+12));
      ...
      for (i = numBlocks; i > 0; i--)
      {
        RijndaelDecrypt(ctx, input, block);
...
int RijndaelPadDecrypt(Rijndael* ctx, const UINT8 *input, int inputOctets, UINT8 *outBuffer)
{
  ...
  if(ctx->m_state != em_Rijndael_Valid)return RIJNDAEL_NOT_INITIALIZED;
  if(ctx->m_direction != em_Rijndael_Decrypt)return RIJNDAEL_BAD_DIRECTION;
...
void RijndaelKeySched(Rijndael* ctx, UINT8 key[_MAX_KEY_COLUMNS][4])
{
  ...
  int uKeyColumns = ctx->m_uRounds - 6;
  ...
  for(j = 0;(j < uKeyColumns) && (r <= ctx->m_uRounds); )
  {
    for(;(j < uKeyColumns) && (t < 4); j++, t++)
    {
      *((UINT32*)ctx->m_expandedKey[r][t]) = *((UINT32*)tempKey[j]);
    }
...
void RijndaelKeyEncToDec(Rijndael* ctx)
{
  ...
  for(r = 1; r < ctx->m_uRounds; r++)
  {
    w = ctx->m_expandedKey[r][0];
    *((UINT32*)w) = *((UINT32*)U1[w[0]]) ^ *((UINT32*)U2[w[1]]) ^ *((UINT32*)U3[w[2]]) ^ *((UINT32*)U4[w[3]]);
    w = ctx->m_expandedKey[r][1];
    *((UINT32*)w) = *((UINT32*)U1[w[0]]) ^ *((UINT32*)U2[w[1]]) ^ *((UINT32*)U3[w[2]]) ^ *((UINT32*)U4[w[3]]);
    w = ctx->m_expandedKey[r][2];
    *((UINT32*)w) = *((UINT32*)U1[w[0]]) ^ *((UINT32*)U2[w[1]]) ^ *((UINT32*)U3[w[2]]) ^ *((UINT32*)U4[w[3]]);
    w = ctx->m_expandedKey[r][3];
    *((UINT32*)w) = *((UINT32*)U1[w[0]]) ^ *((UINT32*)U2[w[1]]) ^ *((UINT32*)U3[w[2]]) ^ *((UINT32*)U4[w[3]]);
  }
}  
...
void RijndaelEncrypt(Rijndael* ctx, const UINT8 a[16], UINT8 b[16])
{
  ...
  UINT8 temp[4][4];
    ...
    *((UINT32*)temp[0]) = *((UINT32*)(a   )) ^ *((UINT32*)ctx->m_expandedKey[0][0]);
    *((UINT32*)temp[1]) = *((UINT32*)(a+ 4)) ^ *((UINT32*)ctx->m_expandedKey[0][1]);
    *((UINT32*)temp[2]) = *((UINT32*)(a+ 8)) ^ *((UINT32*)ctx->m_expandedKey[0][2]);
    *((UINT32*)temp[3]) = *((UINT32*)(a+12)) ^ *((UINT32*)ctx->m_expandedKey[0][3]);
    ...
    *((UINT32*)(b +12)) = *((UINT32*)T1[temp[3][0]])
            ...
            ^ *((UINT32*)T3[temp[1][2]]) 
            ^ *((UINT32*)T4[temp[2][3]]);
  for(r = 1; r < ctx->m_uRounds-1; r++)
  {
    *((UINT32*)temp[0]) = *((UINT32*)(b   )) ^ *((UINT32*)ctx->m_expandedKey[r][0]);
    *((UINT32*)temp[1]) = *((UINT32*)(b+ 4)) ^ *((UINT32*)ctx->m_expandedKey[r][1]);
    *((UINT32*)temp[2]) = *((UINT32*)(b+ 8)) ^ *((UINT32*)ctx->m_expandedKey[r][2]);
    *((UINT32*)temp[3]) = *((UINT32*)(b+12)) ^ *((UINT32*)ctx->m_expandedKey[r][3]);
  ...
  *((UINT32*)temp[0]) = *((UINT32*)(b   )) ^ *((UINT32*)ctx->m_expandedKey[ctx->m_uRounds-1][0]);
  *((UINT32*)temp[1]) = *((UINT32*)(b+ 4)) ^ *((UINT32*)ctx->m_expandedKey[ctx->m_uRounds-1][1]);
  *((UINT32*)temp[2]) = *((UINT32*)(b+ 8)) ^ *((UINT32*)ctx->m_expandedKey[ctx->m_uRounds-1][2]);
  *((UINT32*)temp[3]) = *((UINT32*)(b+12)) ^ *((UINT32*)ctx->m_expandedKey[ctx->m_uRounds-1][3]);
  b[ 0] = T1[temp[0][0]][1];
  b[ 1] = T1[temp[1][1]][1];
  b[ 2] = T1[temp[2][2]][1];
  b[ 3] = T1[temp[3][3]][1];
  ...
  b[14] = T1[temp[1][2]][1];
  b[15] = T1[temp[2][3]][1];
  *((UINT32*)(b   )) ^= *((UINT32*)ctx->m_expandedKey[ctx->m_uRounds][0]);
  *((UINT32*)(b+ 4)) ^= *((UINT32*)ctx->m_expandedKey[ctx->m_uRounds][1]);
  *((UINT32*)(b+ 8)) ^= *((UINT32*)ctx->m_expandedKey[ctx->m_uRounds][2]);
  *((UINT32*)(b+12)) ^= *((UINT32*)ctx->m_expandedKey[ctx->m_uRounds][3]);
}
...
void RijndaelDecrypt(Rijndael* ctx, const UINT8 a[16], UINT8 b[16])
{
  ...
  UINT8 temp[4][4];
    ...
    *((UINT32*)temp[0]) = *((UINT32*)(a   )) ^ *((UINT32*)ctx->m_expandedKey[ctx->m_uRounds][0]);
    *((UINT32*)temp[1]) = *((UINT32*)(a+ 4)) ^ *((UINT32*)ctx->m_expandedKey[ctx->m_uRounds][1]);
    *((UINT32*)temp[2]) = *((UINT32*)(a+ 8)) ^ *((UINT32*)ctx->m_expandedKey[ctx->m_uRounds][2]);
    *((UINT32*)temp[3]) = *((UINT32*)(a+12)) ^ *((UINT32*)ctx->m_expandedKey[ctx->m_uRounds][3]);
  ...
  *((UINT32*)temp[0]) = *((UINT32*)(b   )) ^ *((UINT32*)ctx->m_expandedKey[1][0]);
  *((UINT32*)temp[1]) = *((UINT32*)(b+ 4)) ^ *((UINT32*)ctx->m_expandedKey[1][1]);
  *((UINT32*)temp[2]) = *((UINT32*)(b+ 8)) ^ *((UINT32*)ctx->m_expandedKey[1][2]);
  *((UINT32*)temp[3]) = *((UINT32*)(b+12)) ^ *((UINT32*)ctx->m_expandedKey[1][3]);
  b[ 0] = S5[temp[0][0]];
  b[ 1] = S5[temp[3][1]];
  b[ 2] = S5[temp[2][2]];
  b[ 3] = S5[temp[1][3]];
  ...
  b[14] = S5[temp[1][2]];
  b[15] = S5[temp[0][3]];
  *((UINT32*)(b   )) ^= *((UINT32*)ctx->m_expandedKey[0][0]);
  *((UINT32*)(b+ 4)) ^= *((UINT32*)ctx->m_expandedKey[0][1]);
  *((UINT32*)(b+ 8)) ^= *((UINT32*)ctx->m_expandedKey[0][2]);
  *((UINT32*)(b+12)) ^= *((UINT32*)ctx->m_expandedKey[0][3]);
}
