...
KppUserLog& KppUserLog::UserLogInstance()
{
    ...
    { 
		...
		::GetModuleFileName(HMODULE_THIS, szLogFileName, MAX_PATH -1);
...
HRESULT KppUserLog::ReadOrgLog(std::vector<std::wstring>& vLogCache)
{
	...
	dwRealReadSize = ::fread(pBuf, sizeof(WCHAR), dwFileSize/2, pFile);
	if (dwRealReadSize == 0)
	{
		hrRet = E_FAIL;
		goto _Exit;
	...
	DWORD dwLineCount = _DoGetLineByBuf(pszInfo, ::wcslen(pszInfo), vLogCache);
...
HRESULT KppUserLog::ReadLog(std::multimap<std::wstring, std::wstring>& mapLine)
{
    ...
    dwRealReadSize = ::fread(pBuf, sizeof(BYTE)* 2, dwFileSize/2, pFile);
    //if (dwRealReadSize != 0)
    if (pBuf[0] != 0)
    {
                                  ...
                                  std::vector<std::wstring>& vcStrline)
{
    ...
    while (*pszCurrentPoint != 0 && (pszCurrentPoint - pszBuf) <= dwlen)
    {
        if (*pszCurrentPoint == '\n')
        {
            dwlenSize = pszCurrentPoint - pszBeginPoint;
