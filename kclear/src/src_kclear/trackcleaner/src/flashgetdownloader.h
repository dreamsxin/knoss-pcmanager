					...
					{	
						...
						while (node)
						{
							...
							if(_stricmp(szSelectSize, szDataDownload) == 0)
							{
								...
								wchar_t* lpwShowName = A2W(szShowName);
								CString strFind;
								strFind.Format(_T("%s|Torrents|Torrent|%s"), strPath.c_str(), lpwShowName);
...
//								const char szTorrentFile = 
						...
						while (node)
						{
							...
							if(_stricmp(szSelectSize, "0") == 0)
							{
								...
								wchar_t* lpwShowName = A2W(szShowName);
								CString strFind;
								strFind.Format(_T("%s|Torrents|Download|%s"), strPath.c_str(), lpwShowName);
								g_fnScanFile(g_pMain,BITCOMETDOWNLOADER,strFind,0,0);
								//								const char szTorrentFile = 
				...
				if (rootNode !=NULL)
				{
					...
					{	
						...
						{	
							...
							if (_stricmp(szFinishData, W2A(vec_String[3])) == 0)
							{
