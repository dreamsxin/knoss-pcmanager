...
void CUIHandlerOnekey::_LoadProvider()
{
    ...
    m_hTrashCleaner = ::LoadLibraryW(L"trashcleaner.dll");
    if (m_hTrashCleaner)
    {
        _GetCleaner = (GetCleaner_t)::GetProcAddress(
            m_hTrashCleaner, "GetCleaner");
        if (_GetCleaner)
        {
            _GetCleaner(
                __uuidof(ICleanTask), 
                (void**)&m_piTrashCleaner
                );
        }
    }
...
void CUIHandlerOnekey::_UpdateTitle()
{
    ...
    if (!m_ctrlClean.HasBegin())
    {
		...
		m_dlg->SetItemVisible(ID_DIV_ONEKEY_START, FALSE);
		m_dlg->SetItemText(ID_TXT_ONEKEY_STARTHEAD_2, L"一键清理可以帮你节约磁盘空间，优化注册表，让你的系统运行更有效率。");
