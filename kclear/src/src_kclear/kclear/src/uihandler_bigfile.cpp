...
BOOL CBigfileFilter::LoadFilter()
{
    ...
    xmlDoc.Parse(KUTF16_To_ANSI(strXml.GetBuffer()));
...
BOOL CUIHandlerBigFile::DelBigFile(const CString& strFilePath)
{
    ...
    strTitle = _T("金山卫士");
    uRetCode = msg1.ShowMsg(strMsg, strTitle, MB_OKCANCEL|MB_ICONQUESTION);
    if (uRetCode != IDOK)
        goto clean0;
    ...
    else
    {
        ...
        uRetCode = msg2.ShowMsg(strMsg, strTitle, MB_OK|MB_ICONWARNING);
    }
...
// 弹出一个对话框让用户确认，并且用户可以修改目标路径
BOOL CUIHandlerBigFile::GetMoveDestDir(
    HWND hParentWnd,
...
{
    CBigFileMoveDlg dlg(strSize, strCount, strDestDir, strFileName);
    if (dlg.DoModal(hParentWnd) == IDOK)
    {
        strDestDir = dlg.GetDestDir();
        SaveAutoDestDir(strDestDir);
        return TRUE;
    }
    else
    {
...
BOOL CUIHandlerBigFile::GetAutoDestDir(CString& strDir)
{
    BOOL retval = FALSE;
    BOOL fRetCode;
    TiXmlDocument xmlDoc;
    const TiXmlElement *pXmlSetting = NULL;
    const TiXmlElement *pXmlDestDir = NULL;
    KFilePath settingPath = KFilePath::GetFilePath(_Module.m_hInst);

    strDir = _T("");

    settingPath.RemoveFileSpec();
    settingPath.Append(L"cfg\\bigfilemgr.xml");
    ...
    pXmlDestDir = pXmlSetting->FirstChildElement("destdir");
    if (pXmlDestDir)
    {
        strDir = KUTF8_To_UTF16(pXmlDestDir->Attribute("path"));
    }

reget:
    if (::PathFileExists(strDir.GetBuffer()))
        goto clean1;

    fRetCode = bigfilehelper::GetAutoDestDirFromSize(strDir);
    if (!fRetCode)
        goto clean0;
