	...
	m_fnCallback(NULL), m_lpCallbackParam(NULL), m_bStopScanning(false), m_bCleaned(false)
{
	...
	CRegistryData::SCAN_DATA	data[] = 
	{
		...
		{HKEY_LOCAL_MACHINE, TEXT("Software\\Microsoft\\Windows\\CurrentVersion\\uninstall\\"), TEXT("")}, 
		&CRegistryData::ScanUninstall},
		
		{RST_SHAREDDLL, CRegistryData::GetTitle(RST_SHAREDDLL), 
		{HKEY_LOCAL_MACHINE, TEXT("Software\\Microsoft\\Windows\\CurrentVersion\\SharedDlls\\"), TEXT("")}, 
		&CRegistryData::ScanSharedDlls},
		
		{RST_OPENSAVEMRU, CRegistryData::GetTitle(RST_OPENSAVEMRU), 
		{HKEY_CURRENT_USER, TEXT("Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\ComDlg32\\OpenSaveMRU\\"), TEXT("")}, 
		&CRegistryData::ScanOpenSaveMRU},

		{RST_FILEASSOC, CRegistryData::GetTitle(RST_FILEASSOC), 
		{HKEY_CLASSES_ROOT, TEXT(""), TEXT("")}, 
		&CRegistryData::ScanFileAssociate},

		{RST_COM, CRegistryData::GetTitle(RST_COM), 
		{HKEY_CLASSES_ROOT, TEXT("TypeLib\\"), TEXT("")}, 
		&CRegistryData::ScanTypeLib},

		{RST_COM, CRegistryData::GetTitle(RST_COM), 
		{HKEY_CLASSES_ROOT, TEXT("CLSID\\"), TEXT("")}, 
		&CRegistryData::ScanClsid},

		{RST_EXTHISTORY, CRegistryData::GetTitle(RST_EXTHISTORY), 
		{HKEY_CURRENT_USER, TEXT("Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\FileExts\\"), TEXT("")}, 
		...
		{HKEY_CURRENT_USER, TEXT("Software\\Microsoft\\Windows\\ShellNoRoam\\MUICache\\"), TEXT("")},
		&CRegistryData::ScanMUICache},

		{RST_HELP, CRegistryData::GetTitle(RST_HELP),
		{HKEY_LOCAL_MACHINE, TEXT("SOFTWARE\\Microsoft\\Windows\\Help\\"), TEXT("")},
		&CRegistryData::ScanHelp},

		{RST_HELP, CRegistryData::GetTitle(RST_HELP),
		{HKEY_LOCAL_MACHINE, TEXT("SOFTWARE\\Microsoft\\Windows\\Html Help\\"), TEXT("")},
		&CRegistryData::ScanHelp},

		{RST_APPPATH, CRegistryData::GetTitle(RST_APPPATH),
		{HKEY_LOCAL_MACHINE, TEXT("SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\App Paths\\"), TEXT("")},
...
HRESULT	CRegistryData::ScanUninstall(const CRegistryData::SCAN_DATA& scanData)
{
	...
	EnumRegSubKey_WithCallBack(scanData.scanPoint.hRootKey, scanData.scanPoint.strSubKey.c_str(), 
		&CRegistryData::CheckUninstall, &data);
...
bool CRegistryData::CheckUninstall(HKEY hKey, const TCHAR* lpszSubKeyName, LPVOID param)
{
	HRESULT				hr = E_FAIL;
	bool				bResult = false;
...
HRESULT CRegistryData::CheckDisplayName(const SCAN_DATA& scanData)
{
	...
	bResult = GetRegValueAll(data.scanPoint.hRootKey, data.scanPoint.strSubKey.c_str(), TEXT("DisplayName"), 
		&dwType, (LPBYTE)&bufData[0], &cbData);
	if ( !bResult )
		return E_FAIL;
...
HRESULT	CRegistryData::ScanSharedDlls(const CRegistryData::SCAN_DATA& scanData)
{
	...
	data = scanData;
	data.lpThis = this;
	EnumRegValue_WithCallBack(scanData.scanPoint.hRootKey, scanData.scanPoint.strSubKey.c_str(), 
		&CRegistryData::CheckValueNamePath, &data);
...
bool CRegistryData::CheckValueNamePath(HKEY hKey, const TCHAR* lpszValueName, const void* lpValueData, 
									   DWORD dwType, DWORD dwDataLen, LPVOID param)
{
	HRESULT				hr = E_FAIL;
	bool				bResult = false;
...
HRESULT	CRegistryData::ScanOpenSaveMRU(const CRegistryData::SCAN_DATA& scanData)
{
	...
	data.lpThis = this;
	EnumRegSubKey_WithCallBack(scanData.scanPoint.hRootKey, scanData.scanPoint.strSubKey.c_str(), 
		&CRegistryData::CheckOpenSaveMRUSubKey, &data);
...
bool CRegistryData::CheckOpenSaveMRUSubKey(HKEY hKey, const TCHAR* lpszSubKeyName, LPVOID param)
{
	HRESULT				hr = E_FAIL;
	bool				bResult = false;
	...
	EnumRegValue_WithCallBack(hKey, lpszSubKeyName, &CRegistryData::CheckOpenSaveMRUDataPath, &data);
...
bool CRegistryData::CheckOpenSaveMRUDataPath(HKEY hKey, const TCHAR* lpszValueName, const void* lpValueData, DWORD dwType, DWORD dwDataLen, LPVOID param)
{
	HRESULT				hr = E_FAIL;
	bool				bResult = false;
...
HRESULT	CRegistryData::ScanFileAssociate(const CRegistryData::SCAN_DATA& scanData)
{
	...
	data = scanData;
	data.lpThis = this;
	EnumRegSubKey_WithCallBack(HKEY_CLASSES_ROOT, NULL, &CRegistryData::CheckFileAssociateSubKey, &data);
...
bool CRegistryData::CheckFileAssociateSubKey(HKEY hKey, const TCHAR* lpszSubKeyName, LPVOID param)
{
	HRESULT				hr = E_FAIL;
	bool				bResult = false;
...
bool CRegistryData::CheckExtAssociateKey(const SCAN_DATA& scanData)
{
	...
	if ( !CheckIfKeyEmpty(data.scanPoint.hRootKey, data.scanPoint.strSubKey.c_str()) )
		return true;

	// 取默认值的值
	cbData = (DWORD)bufData.size() * sizeof(TCHAR);
	bResult = GetRegValueAll(data.scanPoint.hRootKey, data.scanPoint.strSubKey.c_str(), data.scanPoint.strValueName.c_str(), 
		&dwType, (LPBYTE)&bufData[0], &cbData);
	if ( !bResult )
	{
		// 若键值不存在, 且无下层子键
		if ( !CheckIfHasSubKey(data.scanPoint.hRootKey, data.scanPoint.strSubKey.c_str()) )
		{
			bDelete = true;
			goto Label_Ret;
		}
	...
	else if ( !CheckIfKeyExist(HKEY_CLASSES_ROOT, &bufData[0]) )
	{
		// 指向的子键HKCR\xx不存在
		bDelete = true;
		goto Label_Ret;
...
bool CRegistryData::CheckDefaultIcon(const SCAN_DATA& scanData)
{
	...
	if ( idx != tstring::npos )
	{
		...
		hr = ::CheckFilePath(scanResult.strInvalidPath);
		if ( hr != S_FALSE )
			return true;
	}
...
HRESULT	CRegistryData::ScanTypeLib(const CRegistryData::SCAN_DATA& scanData)
{
	...
	data = scanData;
	data.lpThis = this;
	EnumRegSubKey_WithCallBack(HKEY_CLASSES_ROOT, scanData.scanPoint.strSubKey.c_str(), 
		&CRegistryData::CheckTypeLibSubKey, &data);
...
bool CRegistryData::CheckTypeLibSubKey(HKEY hKey, const TCHAR* lpszSubKeyName, LPVOID param)
{
	HRESULT				hr = E_FAIL;
	bool				bResult = false;
	...
	// 枚举版本子键
	EnumRegSubKey_WithCallBack(hKey, lpszSubKeyName, &CRegistryData::CheckTypeLibWin32Key, &data);
...
bool CRegistryData::CheckTypeLibWin32Key(HKEY hKey, const TCHAR* lpszSubKeyName, LPVOID param)
{
	HRESULT				hr = E_FAIL;
	bool				bResult = false;
...
HRESULT	CRegistryData::ScanClsid(const CRegistryData::SCAN_DATA& scanData)
{
	...
	data.lpThis = this;
	EnumRegSubKey_WithCallBack(HKEY_CLASSES_ROOT, scanData.scanPoint.strSubKey.c_str(), 
		&CRegistryData::CheckClsidSubKey, &data);
...
bool CRegistryData::CheckClsidSubKey(HKEY hKey, const TCHAR* lpszSubKeyName, LPVOID param)
{
	HRESULT				hr = E_FAIL;
	bool				bResult = false;
	...
	// 枚举版本子键
	EnumRegSubKey_WithCallBack(data.scanPoint.hRootKey, data.scanPoint.strSubKey.c_str(),
		&CRegistryData::CheckClsidCodeBase, &data);
...
bool CRegistryData::CheckClsidCodeBase(HKEY hKey, const TCHAR* lpszSubKeyName, LPVOID param)
{
	HRESULT				hr = E_FAIL;
	bool				bResult = false;
...
HRESULT	CRegistryData::ScanFileExts(const CRegistryData::SCAN_DATA& scanData)
{
	...
	data.lpThis = this;
	EnumRegSubKey_WithCallBack(scanData.scanPoint.hRootKey, scanData.scanPoint.strSubKey.c_str(), 
		&CRegistryData::CheckFileExts, &data);
...
bool CRegistryData::CheckFileExts(HKEY hKey, const TCHAR* lpszSubKeyName, LPVOID param)
{
	HRESULT				hr = E_FAIL;
	bool				bResult = false;
	...
	if ( !CheckIfKeyExist(HKEY_CLASSES_ROOT, lpszSubKeyName) )
	{
		bHKCRKeyExist = false;
	}
	else
	{
		...
		bListEmpty = CheckIfKeyEmptyOrNonExist(data.scanPoint.hRootKey, strKey.c_str());

		// OpenWithProgids
		strKey = data.scanPoint.strSubKey;
		strKey += TEXT("OpenWithProgids");
		bProgidEmpty = CheckIfKeyEmptyOrNonExist(data.scanPoint.hRootKey, strKey.c_str());
...
HRESULT CRegistryData::ScanOfficeOpenHistory(const SCAN_DATA& scanData)
{
	...
	data.lpThis = this;
	EnumRegSubKey_WithCallBack(scanData.scanPoint.hRootKey, scanData.scanPoint.strSubKey.c_str(), 
		&CRegistryData::CheckOfficeOpenHistory, &data);
...
bool CRegistryData::CheckOfficeOpenHistory(HKEY hKey, const TCHAR* lpszSubKeyName, LPVOID param)
{
	HRESULT				hr = E_FAIL;
	bool				bResult = false;
	...
	for ( int i = 0; i < sizeof(lpListKey)/sizeof(lpListKey[i]); ++i )
	{
		...
		// 检查子键是否存在
		if ( !CheckIfKeyExist(scanResult.scanData.hRootKey, scanResult.scanData.strSubKey.c_str()) )
			continue;
...
HRESULT CRegistryData::ScanMUICache(const SCAN_DATA& scanData)
{
	...
	data.lpThis = this;
	EnumRegValue_WithCallBack(scanData.scanPoint.hRootKey, scanData.scanPoint.strSubKey.c_str(), 
		&CRegistryData::CheckMUICache, &data);
...
bool CRegistryData::CheckMUICache(HKEY hKey, const TCHAR* lpszValueName, const void* lpValueData, DWORD dwType, DWORD dwDataLen, LPVOID param)
{
	HRESULT				hr = E_FAIL;
	bool				bResult = false;
...
bool CRegistryData::CheckHelp(HKEY hKey, const TCHAR* lpszValueName, const void* lpValueData, DWORD dwType, DWORD dwDataLen, LPVOID param)
{
	HRESULT							hr = E_FAIL;
	bool							bResult = false;
	...
	hr = ::CheckFilePath(strAppend, CRegistryData::PT_FILE);
	if ( hr == S_OK )
		return true;
...
HRESULT CRegistryData::ScanAppPath(const SCAN_DATA& scanData)
{
	...
	data.lpThis = this;
	EnumRegSubKey_WithCallBack(scanData.scanPoint.hRootKey, scanData.scanPoint.strSubKey.c_str(), 
		&CRegistryData::CheckAppPath, &data);
...
bool CRegistryData::CheckAppPath(HKEY hKey, const TCHAR* lpszSubKeyName, LPVOID param)
{
	HRESULT				hr = E_FAIL;
	bool				bResult = false;
...
HRESULT CRegistryData::ScanMenuOrder(const SCAN_DATA& scanData)
{
    ...
    data.lpThis = this;
    EnumRegSubKey_WithCallBack(scanData.scanPoint.hRootKey, scanData.scanPoint.strSubKey.c_str(), 
        &CRegistryData::CheckMenuOrder, &data);
...
bool CRegistryData::CheckMenuOrder(HKEY hKey, const TCHAR* lpszSubKeyName, LPVOID param)
{
    HRESULT				hr = E_FAIL;
    bool				bResult = false;
    ...
    if ( CheckIfMenuItemExist(lpszSubKeyName) )
        return true;
...
HRESULT	CRegistryData::CheckCmdline(SCAN_RESULT& scanResult)
{
	...
	if ( scanResult.ppPos == PP_VALUE_DATA )
	{
		...
		cbData = (DWORD)bufData.size() * sizeof(TCHAR);
		bResult = GetRegValueAll(scanResult.scanData.hRootKey, scanResult.scanData.strSubKey.c_str(),  
			scanResult.scanData.strValueName.c_str(), &dwType, (LPBYTE)&bufData[0], &cbData);
		if ( !bResult )
			return E_FAIL;
...
HRESULT	CRegistryData::CheckFilePath(SCAN_RESULT& scanResult)
{
	...
	// 检查路径
	hr = ::CheckFilePath(strPath, scanResult.ptPath);
...
//
//////////////////////////////////////////////////////////////////////////
bool DoesFileExist(IN const TCHAR* lpszFilePath)
{
	bool				bReturn = false;
...
bool ExpandPath(TCHAR *lpszDllPath)
{
	bool					bReturn = false;
	bool					bResult = false;
	UINT					uResult = 0;
...
// 若子键不存在或为空子键，返回true
bool CheckIfKeyEmptyOrNonExist(HKEY hKey, const TCHAR* lpszSubKey)
{
	bool		bReturn = false;
	HKEY		hSubKey = NULL;
	LONG		lResult = ERROR_SUCCESS;
...
bool CheckIfKeyEmpty(HKEY hKey, const TCHAR* lpszSubKey)
{
	bool		bReturn = false;
	HKEY		hSubKey = NULL;
	LONG		lResult = ERROR_SUCCESS;
...
// 若有下层子键，返回true
bool CheckIfHasSubKey(HKEY hKey, const TCHAR* lpszSubKey)
{
	bool		bReturn = false;
	HKEY		hSubKey = NULL;
	LONG		lResult = ERROR_SUCCESS;
...
// 检查子键是否存在, 存在返回true
bool CheckIfKeyExist(HKEY hKey, const TCHAR* lpszSubKey)
{
	bool		bReturn = false;
	HKEY		hSubKey = NULL;
	LONG		lResult = ERROR_SUCCESS;
...
bool CheckIfMenuItemExist(const tstring& strItemPath)
{
    bool                bReturn = false;
...
HRESULT	CheckFilePath(tstring& strPath, CRegistryData::PATH_TYPE ptPath)
{
	HRESULT				hr = E_FAIL;
	std::vector<TCHAR>	bufFile(3*MAX_PATH, 0);
	LPCTSTR				lpValueName = NULL;
	...
	// 判断文件是否存在
	if ( (ptPath == CRegistryData::PT_FILE || ptPath == CRegistryData::PT_ALL) && DoesFileExist(&bufFile[0])  )
		return S_OK;
...
HRESULT	CRegistryData::CleanKey(const REG_DATA& scanData)
{
	...
	lRetCode = regKey.Open(scanData.hRootKey, scanData.strSubKey.c_str(), READ_CONTROL|WRITE_DAC);
	if (lRetCode)
		goto clean0;
	...
	lRetCode = regKey.Open(scanData.hRootKey, scanData.strSubKey.c_str(), KEY_ALL_ACCESS);
	if (lRetCode)
		goto clean0;
	...
	if ( ERROR_SUCCESS == hr )
	{
// 		dwHandle = (unsigned int)(ULONG_PTR)scanData.hRootKey;
// 		wsprintf(szTemp, L"0x%x:", dwHandle);					// 2+8+1+1
// 		strLog = szTemp;
// 		strLog += L"\\";
// 		strLog += scanData.strSubKey;
// 		KppUserLog::UserLogInstance().WriteSingleLog(strLog);
	}
	regKey.Close();
