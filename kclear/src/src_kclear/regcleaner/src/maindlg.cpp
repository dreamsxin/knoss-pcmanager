...
CMainDlg::CMainDlg() :
	m_bStop(false), m_hScanThread(NULL), m_scStatus(CMainDlg::SCS_IDLE),
...
void CMainDlg::SetScStatus(SC_STATUS status)
{
	...
	if ( status == CMainDlg::SCS_IDLE )
	{
		...
		::SetWindowText(m_txtDesc, TEXT("清理注册表可加快系统速度，建议定期清理！（清理之后，注册表会自动备份，以便恢复）"));
...
LRESULT CMainDlg::OnCleanClick(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL &bHandled)
{
	...
	// 创建清除线程
	m_hScanThread = ::CreateThread(NULL, 0, &CMainDlg::CleanThreadProc, this, 0, NULL);
	if ( m_hScanThread == NULL )
	{
