...
{
	PVOID pResponseContext;			//由驱动给出，防火墙主程序发送响应时，原值传回。
...
typedef struct tagExtrDataArpData
{
	...
	unsigned char  gateway_tha[6];         /* gateway hardware address */
	unsigned long  gateway_tpa;			   /* gateway protocol address */
	unsigned char  local_tha[6];         /* local hardware address */
	unsigned long  local_tpa[MAX_ADAPTER_IP_NUM];	/* local protocol address */
