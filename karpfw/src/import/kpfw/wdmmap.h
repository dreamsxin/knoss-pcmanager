...
template< typename T , BOOLEAN LOCK = TRUE ,int HASH_SIZE = 1000 >
class KWDMMap
{
public:
	typedef struct _Hash_Node<T> Hash_Node;
	typedef struct _Hash_Node<T>* PHash_Node;
	struct Iterator
	{
...
public:
	...
	*/
	...
	BOOLEAN insert(PVOID ulKey, T ulPtrValue)
	{
		KIRQL oldirq;
...
EXIT0:	
	...
	BOOLEAN erase_nolock(PVOID ulKey, T* tValue)
	{
		PHash_Node pEntry = NULL;
	...
	BOOLEAN erase(PVOID ulKey, T* tValue)
	{
		KIRQL oldirq;
		BOOLEAN bFind = FALSE;
	...
	BOOLEAN find(PVOID ulKey ,  T* tValue)
	{
		PHash_Node pEntry = NULL;
		if ( NULL == tValue)
		{
	...
	BOOLEAN find_lock(PVOID ulKey ,  T* tValue)
	{
		KIRQL oldirq;
		BOOLEAN bFind = FALSE;
		KeAcquireSpinLock(&m_spinLock , & oldirq);
