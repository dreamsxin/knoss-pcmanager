...
public:
    ...
    inline int sgetn(__out_ecount(cbBuf) char *,int cbBuf);

    virtual int sync();

    virtual streambuf* setbuf(__in_opt char *, int);
    virtual streampos seekoff(streamoff,ios::seek_dir,int =ios::in|ios::out);
    virtual streampos seekpos(streampos,int =ios::in|ios::out);

    virtual int xsputn(const char *,int);
    virtual int xsgetn(__out_ecount(cbBuf) char *,int cbBuf);
...
protected:
    ...
    inline void setp(__in char *,__in char *);
    inline void setg(__in char *,__in char *,__in char *);
    inline void pbump(int);
    inline void gbump(int);

    void setb(__in_ecount(eb-b) char *b,__in_ecount(0) char *eb,int =0);
...
inline int streambuf::sgetn(__out_ecount(_n) char * _str,int _n) { return xsgetn(_str, _n); }
...
inline void streambuf::setg(__in char * _eb, __in char * _g, __in char * _eg) {_eback=_eb; _gptr=_g; _egptr=_eg; x_lastc=EOF; }
inline void streambuf::setp(__in char * _p, __in char * _ep) {_pptr=_pbase=_p; _epptr=_ep; }
