...
public:
    ...
    _CRTIMP_TYPEINFO __TYPEINFO_OPERATOR_EQ_RET _THISCALL operator==(__in const type_info& _Rhs) const;
    _CRTIMP_TYPEINFO __TYPEINFO_OPERATOR_EQ_RET _THISCALL operator!=(__in const type_info& _Rhs) const;
    _CRTIMP_TYPEINFO int _THISCALL before(__in const type_info& _Rhs) const;
    _CRTIMP_TYPEINFO __checkReturn const char* _THISCALL name() const;
    _CRTIMP_TYPEINFO __checkReturn const char* _THISCALL raw_name() const;
private:
    void *_m_data;
    char _m_d_name[1];
    type_info(__in const type_info& _Rhs);
    type_info& operator=(__in const type_info& _Rhs);
};
...
// This function is not exported prior to Windows XP.
    __CLR_OR_THIS_CALL bad_cast(__in_z const char * _Message = "bad cast") : exception(_Message) { }
#else
