...
--*/
{
    ...
    UNREFERENCED_PARAMETER(SystemSpecific2);
     ...
     KAntiarpDebugPrint(TRACE_LEVEL_INFORMATION, FLAG_NDIS_PROTOCOL, "PtBindAdapter++. DeviceName %ws", DeviceName->Buffer);
    ...
    do
    {
        ...
        // instance string.
        //
        TotalSize = sizeof(KANTIARP_ADAPTER) 
        ...
        pAdapt->DeviceName.Buffer = (PWCHAR)((ULONG_PTR)pAdapt + sizeof(KANTIARP_ADAPTER));
        NdisMoveMemory(pAdapt->DeviceName.Buffer,
                       Param->ParameterData.StringData.Buffer,
                       Param->ParameterData.StringData.MaximumLength);

		
		// save upper device name (used by GetIpFromReg )
		pAdapt->UpperDeviceName.MaximumLength = DeviceName->MaximumLength;
		pAdapt->UpperDeviceName.Length = DeviceName->Length;
		pAdapt->UpperDeviceName.Buffer = (PWCHAR)((ULONG_PTR)pAdapt + sizeof(KANTIARP_ADAPTER)
			+ Param->ParameterData.StringData.MaximumLength  
			+ sizeof(WCHAR) );
		NdisMoveMemory(
			pAdapt->UpperDeviceName.Buffer,
			DeviceName->Buffer,
			DeviceName->MaximumLength
			);

		NdisInitializeEvent( &pAdapt->hQueryEvent );
		...
		//added by Yzh
		NdisAllocateBufferPool( Status, &pAdapt->hSendBufferPool, 0x40 );
		if (*Status != NDIS_STATUS_SUCCESS)
		{
			break;
		}

		NdisAllocateBufferPool( Status, &pAdapt->hRecvBufferPool, 0x40 );
		if (*Status != NDIS_STATUS_SUCCESS)
		{
			break;
		}
...
//#pragma prefast(suppress: __WARNING_POTENTIAL_BUFFER_OVERFLOW, "Ndis guarantees MediumIndex to be within bounds");
        ...
        *Status = NdisIMInitializeDeviceInstanceEx(g_Data.DriverHandle,
                                           &pAdapt->DeviceName,
                                           pAdapt);
        ...
        if (*Status != NDIS_STATUS_SUCCESS)
        {
            ...
            KAntiarpDebugPrint(TRACE_LEVEL_ERROR, FLAG_NDIS_PROTOCOL, 
					"PtBindAdapter %x", *Status );
    ...
    KAntiarpDebugPrint(TRACE_LEVEL_INFORMATION, FLAG_NDIS_PROTOCOL, "PtBindAdapter--. STATUS = %x", *Status);
}
...
--*/
{
    PKANTIARP_ADAPTER      pAdapt =(PKANTIARP_ADAPTER)ProtocolBindingContext;
...
--*/
{
	...
	KAntiarpDebugPrint(TRACE_LEVEL_INFORMATION, FLAG_NDIS_PROTOCOL, "PtBindAdapter++ %p", pAdapt );
    ...
    else
    {
		...
		if( pAdapt->arpCache != NULL )
		{
			non_paged_pool::__DeallocateSystemMemory( (PVOID)pAdapt->arpCache );
			pAdapt->arpCache = NULL;
		}
...
)
{
	...
	KAntiarpDebugPrint(TRACE_LEVEL_INFORMATION, FLAG_NDIS_PROTOCOL, "PtUnloadProtocol++" );
    if (g_Data.ProtocolHandle != NULL)
    {
        NdisDeregisterProtocol(&Status, g_Data.ProtocolHandle);
        g_Data.ProtocolHandle = NULL;
    }
    ...
    switch (NdisRequest->RequestType)
    {
      case NdisRequestQueryInformation:
        ...
        *pAdapt->BytesNeeded = NdisRequest->DATA.QUERY_INFORMATION.BytesNeeded;
		...
		NdisSetEvent( &pAdapt->hQueryEvent );
...
{
    PKANTIARP_ADAPTER      pAdapt = (PKANTIARP_ADAPTER)ProtocolBindingContext;
	PNDIS_WAN_LINE_UP ndisWanLineUp = NULL;
	PNDIS_WAN_LINE_DOWN ndisWanLineDown = NULL;
    ...
    //
    if ((pAdapt->MiniportHandle != NULL)  &&
        (pAdapt->MiniportPowerState == NdisDeviceStateD0) &&
        (pAdapt->PTDeviceState == NdisDeviceStateD0))    
    {
		...
		case NDIS_STATUS_MEDIA_DISCONNECT:
			{
				if( pAdapt->arpCache != NULL )
				{
					RtlZeroMemory( (char*)pAdapt->arpCache, sizeof( ARP_CACHE) * MAX_ARP_CACHE );
					RtlZeroMemory( pAdapt->GatewayMac, 6 );
				}
			}
			//no break
		...
		case NDIS_STATUS_WAN_LINE_UP:
			{
				g_ndisFirewall->WanLineUp( (PNDIS_WAN_LINE_UP)StatusBuffer );
			}
			break;
		case NDIS_STATUS_WAN_LINE_DOWN:
			{
				g_ndisFirewall->WanLineDown( (PNDIS_WAN_LINE_DOWN)StatusBuffer );
			}
			break;
		}
...
--*/
{
	...
	PVOID pOurBuffer = NULL;
	ULONG uBufferLen = 0;
	PNDIS_BUFFER pUserBuffer = NULL;
...
#endif // NDIS51
    {
		...
		if( Pkt == NULL )
		{
			NdisUnchainBufferAtFront( Packet, &pUserBuffer ); 
			NdisFreeBuffer( pUserBuffer );
...
--*/
{
    ...
    if( pAdapt->MiniportHandle )
    {
		...
		else
		{
			...
			NdisQueryBufferSafe( pPacketBuffer, pPacketMem, &uPackSize, NormalPagePriority );
			pPacketMem -= RecvRsvd->uPackSize;		//包括头的内容
			uPackSize += RecvRsvd->uPackSize ;		//加上头的长度

			if( g_ndisFirewall != NULL
				&& g_ndisFirewall->Filter( pAdapt, pPacketMem, uPackSize, enumPD_Recieve ) != enumIP_Block )
			{
				NdisFreeBuffer( pPacketBuffer );

				//重新分配
				NdisAllocateBuffer( &Status, &pPacketBuffer, pAdapt->RecvPacketPoolHandle, pPacketMem, uPackSize ); 
			...
			NdisFreeBuffer( pPacketBuffer );
			non_paged_pool::__DeallocateSystemMemory( pPacketMem );
			NdisFreePacket( Packet );
		}
	}
}
...
--*/
{
	...
	PNDIS_BUFFER		pPacketBuf = NULL;
	PVOID				pRecvBuffer = NULL;

    if ((!pAdapt->MiniportHandle) || (pAdapt->MiniportPowerState > NdisDeviceStateD0))
    {
        Status = NDIS_STATUS_FAILURE;
    }
    else do
    {
        ...
        if (Packet != NULL)
        {
            ...
            if (Status == NDIS_STATUS_SUCCESS)
            {
                ...
                //
				if( g_ndisFirewall != NULL
					&& g_ndisFirewall->FilterPackets( pAdapt, MyPacket, enumPD_Recieve ) != enumIP_Block )
				{
					// indicate packet up
		...
		else if (LookAheadBufferSize >= PacketSize)
		{
			...
			pRecvBuffer = non_paged_pool::__AllocateSystemMemory( PacketSize + HeaderBufferSize );
			if( pRecvBuffer == NULL )
				return  NDIS_STATUS_RESOURCES;

			RtlCopyMemory( pRecvBuffer, HeaderBuffer, HeaderBufferSize );
			RtlCopyMemory( (UCHAR*)pRecvBuffer + HeaderBufferSize, LookAheadBuffer, PacketSize );

			if( g_ndisFirewall != NULL
				&& g_ndisFirewall->Filter( pAdapt, pRecvBuffer, PacketSize + HeaderBufferSize, enumPD_Recieve ) == enumIP_Block )
				Status = NDIS_STATUS_FAILURE;

			if( pRecvBuffer != NULL )
			{
				non_paged_pool::__DeallocateSystemMemory( pRecvBuffer );
				pRecvBuffer = NULL;
			}
		...
		else 
		{
			do 
			{
				...
				pRecvBuffer = non_paged_pool::__AllocateSystemMemory( HeaderBufferSize + PacketSize );
				if ( pRecvBuffer == NULL )
				{
					Status = NDIS_STATUS_RESOURCES;
					break;
				...
				NdisMoveMemory( pRecvBuffer, HeaderBuffer, HeaderBufferSize );
				NdisMoveMemory( (UCHAR*)pRecvBuffer + HeaderBufferSize, LookAheadBuffer, LookAheadBufferSize );

				NdisAllocateBuffer(
					&Status,
					&pPacketBuf,
					pAdapt->hRecvBufferPool, 
					(UCHAR*)pRecvBuffer + HeaderBufferSize + LookAheadBufferSize,
					PacketSize - LookAheadBufferSize );
				if ( Status != NDIS_STATUS_SUCCESS )
					break;

				NdisChainBufferAtFront( MyPacket, pPacketBuf );
				...
				RecvRsvd->uPackSize = HeaderBufferSize + LookAheadBufferSize;
				RecvRsvd->uNeedSize = PacketSize - LookAheadBufferSize;
				...
				NdisUnchainBufferAtFront( MyPacket, &pPacketBuf );
				NdisAllocateBuffer(
					&Status,
					&pPacketBuf,
					pAdapt->hRecvBufferPool, 
...
--*/
{
	...
	KAntiarpDebugPrint(TRACE_LEVEL_INFORMATION, 
		FLAG_NDIS_PROTOCOL, "PtPNPHandler++ Adapt %p, Event %d",
	...
	KAntiarpDebugPrint(TRACE_LEVEL_INFORMATION, FLAG_NDIS_PROTOCOL, "PtPNPHandler-- Adapt %p", pAdapt );
...
--*/
{
	...
	KAntiarpDebugPrint(TRACE_LEVEL_INFORMATION, FLAG_NDIS_PROTOCOL, "PtPnPNetEventReconfigure++ Adapt %p", pAdapt );
    ...
    do
    {
        ...
        if (pAdapt == NULL)
        {
            ...
            //
            NdisReEnumerateProtocolBindings (g_Data.ProtocolHandle);        
...
--*/
{
	...
	KAntiarpDebugPrint(TRACE_LEVEL_INFORMATION, FLAG_NDIS_PROTOCOL, "PtPnPNetEventSetPower++ Adapt %p", pAdapt );
