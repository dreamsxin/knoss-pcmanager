...
{
    UINT            i;
    PKANTIARP_ADAPTER          pAdapt;
    NDIS_STATUS     Status = NDIS_STATUS_FAILURE;
    NDIS_MEDIUM     Medium;

    UNREFERENCED_PARAMETER(WrapperConfigurationContext);
    
	KAntiarpDebugPrint(TRACE_LEVEL_INFORMATION, FLAG_NDIS_MINIPORT, "MPInitialize++ " );
    ...
    do
    {
        ...
        //
        pAdapt = (PKANTIARP_ADAPTER)NdisIMGetDeviceContext(MiniportAdapterHandle);
        pAdapt->MiniportIsHalted = FALSE;

		KAntiarpDebugPrint(TRACE_LEVEL_INFORMATION, FLAG_NDIS_MINIPORT, "MPInitialize pAdapt:%p", pAdapt );
		...
		KNdisFirewall::GetAdapterMac( pAdapt );
		KNdisFirewall::GetAdapterPhysicalMedium( pAdapt );
		KNdisFirewall::GetAdapterStatus( pAdapt );
		if ( pAdapt->Medium == NdisMedium802_3 )
		{
			pAdapt->arpCache = (PARP_CACHE)non_paged_pool::__AllocateSystemMemory( sizeof( ARP_CACHE) * MAX_ARP_CACHE );
        ...
        pAdapt->Next = g_Data.AdapterList;
        g_Data.AdapterList = pAdapt;
	...
	KAntiarpDebugPrint(TRACE_LEVEL_INFORMATION, FLAG_NDIS_MINIPORT, "MPInitialize-- pAdapt:%p Status:%x", pAdapt, Status );
...
--*/
{
    PKANTIARP_ADAPTER              pAdapt = (PKANTIARP_ADAPTER)MiniportAdapterContext;
    NDIS_STATUS         Status;
    ...
    // power state
    //
    if (pAdapt->MiniportPowerState > NdisDeviceStateD0)
    {
        ...
        do 
        {
            ...
            if (Status == NDIS_STATUS_SUCCESS)
            {
				...
				if( g_ndisFirewall != NULL 
					&& g_ndisFirewall->FilterPackets( pAdapt, MyPacket, enumPD_Send ) != enumIP_Block )
				{
					// forward the packet
					NdisSend(
						&Status,
...
MPQueryPNPCapabilities(
    IN OUT PKANTIARP_ADAPTER            pAdapt,
...
--*/
{
	...
	KAntiarpDebugPrint(TRACE_LEVEL_INFORMATION, FLAG_NDIS_MINIPORT, "MPProcessSetPowerOid++ %p", pAdapt );
    ...
    *pNdisStatus = NDIS_STATUS_FAILURE;
    ...
    do 
    {
        ...
        //
        if ((pAdapt->MiniportPowerState > NdisDeviceStateD0) && (NewDeviceState != NdisDeviceStateD0))
        {
            //
            // If the miniport is in a non-D0 state, the miniport can only receive a Set Power to D0
            //
            ASSERT (!(pAdapt->MiniportPowerState > NdisDeviceStateD0) && (NewDeviceState != NdisDeviceStateD0));
        ...
        //
        pAdapt->MiniportPowerState = NewDeviceState;
...
--*/
{
    PKANTIARP_ADAPTER             pAdapt = (PKANTIARP_ADAPTER)MiniportAdapterContext;
    NDIS_STATUS        Status;
    PKANTIARP_ADAPTER            *ppCursor;

	KAntiarpDebugPrint(TRACE_LEVEL_INFORMATION, FLAG_NDIS_MINIPORT, "MPHalt++ %p", pAdapt );
    ...
    for (ppCursor = &g_Data.AdapterList; *ppCursor != NULL; ppCursor = &(*ppCursor)->Next)
    {
        if (*ppCursor == pAdapt)
        {
