    ...
    void OnPaint(CDCHandle dc)
    {
        ...
        if (m_bShowPercent)
        {
            ...
            if (CLR_INVALID == m_style.m_crBg)
                ...
                OldClr = dc.SetTextColor(m_style.m_crBg);
...
public:
    CBkRealWnd()
        : m_uRealWinDlgResID(0), m_hWndReal(NULL)
    {
    ...
    void ShowRealWindow(BOOL bShow)
    {
		...
		if (rc != m_rcWindow)
		{            
			::SetWindowPos(_GetRealWindow(), HWND_TOP, m_rcWindow.left, m_rcWindow.top, m_rcWindow.Width(), m_rcWindow.Height(), SWP_NOREDRAW);
		}
    ...
    void OnWindowPosChanged(LPWINDOWPOS lpWndPos)
    {
        ...
        if (rcOldWnd != m_rcWindow)
        {
            ...
            if (::IsWindowVisible(_GetRealWindow()))
            {
...
//                 ::RedrawWindow(hWndReal, NULL, NULL, RDW_ERASE | RDW_FRAME | RDW_INVALIDATE | RDW_ERASENOW);
                ::InvalidateRect(_GetRealWindow(), NULL, TRUE);
                ::SetWindowPos(_GetRealWindow(), NULL, 0, 0, 0, 0, SWP_NOSIZE | SWP_NOMOVE | SWP_NOZORDER | SWP_SHOWWINDOW | SWP_FRAMECHANGED);
            }
        }
    }
...
protected:
    ...
    HWND _GetRealWindow()
    {
		if (NULL == m_hWndReal)
		{
			m_hWndReal = ::GetDlgItem(m_hWndContainer, m_uRealWinDlgResID);
		}
