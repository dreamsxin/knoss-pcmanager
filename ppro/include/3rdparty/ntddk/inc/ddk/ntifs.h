/*++ BUILD Version: 0145    // Increment this if a change has global effects
...
//
//  Tag allocated to SOLUTIONSOFT for HSM
//
...
#define IO_REPARSE_TAG_WISDATA_HSM              (0x00000010L)
...
#if (NTDDI_VERSION >= NTDDI_LONGHORN || (NTDDI_VERSION >= NTDDI_WINXPSP2 && NTDDI_VERSION < NTDDI_WS03))
NTKERNELAPI
BOOLEAN
...
#define FSRTL_NETWORK1_TOP_LEVEL_IRP    0x05
#define FSRTL_NETWORK2_TOP_LEVEL_IRP    0x06
...
typedef UUID UOW, *PUOW;
typedef GUID *PGUID;
typedef ULONG KSAVEPOINT, *PKSAVEPOINT;
...
typedef UUID KCRM_PROTOCOL_ID, *PKCRM_PROTOCOL_ID;
...
TmSavepointTransaction (
    IN PKTRANSACTION  Transaction,
    IN BOOLEAN        Wait,
    OUT PKSAVEPOINT   Savepoint
    );

NTSTATUS
TmClearSavepointTransaction (
    IN PKTRANSACTION  Transaction,
    IN KSAVEPOINT     SavepointId
    );


NTSTATUS
TmClearAllSavepointsTransaction (
    IN PKTRANSACTION Transaction
    );


NTSTATUS
TmRollbackSavepointTransaction (
    IN PKTRANSACTION Transaction,
    IN KSAVEPOINT    SavepointId
    );
...
NTKERNELAPI
ULONG
TmRmNotificationCount_Temporary (
...
TmPullTransaction(
    ...
    OUT PVOID*                PropagationCookie,
    IN  ULONG                 NumberOfProtocols,
    IN  PKCRM_PROTOCOL_ID     ProtocolArray,
    IN  ULONG                 BufferLength,
    IN  PVOID                 Buffer,
    IN  PTM_PROPAGATE_ROUTINE Callback,
    IN  PVOID                 CallbackData
    );
...
//
// Initialize tm object types (phase 1).
//
...
//
// Initialize tm data structures (phase 2).
//
...
//
// Initialize default TM.  TODO: REMOVE THIS BEFORE SHIPPING
//

NTSTATUS
TmInitDefaultTemporaryTm(
    IN PDEVICE_OBJECT DeviceObjectHint,
    IN ULONG          HintSize
    );
...
TmCurrentTransaction (
    __out PHANDLE CurrentTransaction
    );
...
TmFreezeTransactions (
    __in_opt PLARGE_INTEGER FreezeTimeout,
    __in_opt PLARGE_INTEGER ThawTimeout,
