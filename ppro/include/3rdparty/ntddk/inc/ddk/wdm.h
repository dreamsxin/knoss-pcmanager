/*++ BUILD Version: 0161    // Increment this if a change has global effects
...
NtPullTransaction(
    ...
    IN  PCRM_PROTOCOL_ID   ProtocolArray OPTIONAL,
...
#endif // (((_NTDRIVER_) || (_NTDDK_) || (_NTIFS_) || (_NTHAL_)) && !(_NTSYSTEM_DRIVER_) || (_NTOSP_))
...
KeStartDynamicProcessor (
    __in ULONG ProcessorId,
    __in ULONG ApicId,
    __out PULONG NtNumber
    );
...
} KE_PROCESSOR_ADD_NOTIFY_STATE;

typedef struct _KE_PROCESSOR_ADD_NOTIFY_CONTEXT {
    KE_PROCESSOR_ADD_NOTIFY_STATE State;
    ULONG NtNumber;
    NTSTATUS Status;
} KE_PROCESSOR_ADD_NOTIFY_CONTEXT, *PKE_PROCESSOR_ADD_NOTIFY_CONTEXT;
...
IoCallDriverStackSafe(
    __in PDEVICE_OBJECT DeviceObject,
    __inout PIRP Irp,
    __in ULONG StackThreshold,
    __in ULONG FailIfNoSwitch
    );
#endif

//
// Flags for IoCallDriverStackSafe
//
#define IO_FAIL_NO_STACK_SWITCH        0x00000001

#define IoCallDriverStackSafeDefault(a, b) \
        IoCallDriverStackSafe(a, b, 0, 0)
...
typedef struct _EXPRESS_AER_CAPABILITY {
...
} EXPRESS_AER_CAPABILITY, *PEXPRESS_AER_CAPABILITY;

//
// Advanced Error Reporting Capability structure for root port.
//

typedef struct _EXPRESS_ROOTPORT_AER_CAPABILITY {
...
} EXPRESS_ROOTPORT_AER_CAPABILITY, *PEXPRESS_ROOTPORT_AER_CAPABILITY;
...
ZwQueryInformationTransactionManager (
    __in HANDLE TransactionManagerHandle,
    __in TRANSACTION_INFORMATION_CLASS TransactionManagerInformationClass,
