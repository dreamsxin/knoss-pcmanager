...
#define XX_MSG_DLG_HEIGHT	96
	...
	void OnLButtonDown(UINT uMsg, CPoint ptx)
	{
		...
		if (pdata!=NULL && pdata->pInfo)
		{
			...
			if (iIndex == CLICK_INDEX_ENABLE || iIndex== CLICK_INDEX_OPEN_FOLDER || iIndex == CLICK_INDEX_DELETE)
			{
		...
		if (pInfo && pItemData)
		{
			...
			if (pItemData->nType == TYPE_ITEM_MAIN )
			{
				...
				if (TRUE)
				{
					...
					{
						dcx.SetTextColor(COL_STR_FILENOTEXIST);
						dcx.DrawText(strAdvice,-1,&rcState,DT_VCENTER|DT_SINGLELINE|DT_CENTER);
						dcx.SetTextColor(pInfo->bEnable?COL_STR_CPT_DEFAULT:COL_STR_CPT_DISABLE);
					}
					else
					{
					...
					{
						if (pInfo->bFileIsExist)
							strState.Format(_T("%s"), pInfo->bEnable?BkString::Get(IDS_SYSOPT_4038):BkString::Get(IDS_SYSOPT_4039));
						else
					...
					{
						if (pInfo->bFileIsExist)
							strState.Format(_T("%s"), pInfo->bEnable?BkString::Get(IDS_SYSOPT_4037):BkString::Get(IDS_SYSOPT_4036));
						else
							strState.Format(_T("%s"), BkString::Get(IDS_SYSOPT_4062));
					}
...
#define LENGTH_OPEN_FOLDER	50
						...
						rcOpenDir.left = rcApp.left;
						rcOpenDir.top = rcApp.bottom + MARGIN_OPEN_FOLDER + 2;
						rcOpenDir.bottom = rcOpenDir.top + MARGIN_OPEN_FOLDER + 2;
						...
						if (pInfo->nType != 2)	//针对正常的服务项，不提供“删除此项”LinkText
						{
							//“删除此项”LinkText
							CRect rcDeleteItem = rcOpenDir;
							rcDeleteItem.left = rcOpenDir.left + LENGTH_OPEN_FOLDER*2;
							rcDeleteItem.right = rcDeleteItem.left + LENGTH_OPEN_FOLDER;

							strShow.Format(_T("%s"),BkString::Get(IDS_SYSOPT_4053));
							dcx.DrawText(strShow,-1,&rcDeleteItem,DT_VCENTER|DT_SINGLELINE|DT_LEFT);
							pItemData->rcDelete = rcDeleteItem;
							pItemData->rcDelete.OffsetRect(-lpDrawItemStruct->rcItem.left,-lpDrawItemStruct->rcItem.top);
						}
						else
						{
							//针对目标文件已不存在的服务项，提供“删除此项”LinkText
							if (pInfo->bFileIsExist == FALSE)
							{
								//“删除此项”LinkText
								CRect rcDeleteItem = rcOpenDir;
	...
	void SetShowDlgState()
	{
		if (XBASE_GetItemCount()==0)
		{
			...
			else if ( m_nShowType==KSRUN_TYPE_TASK )
			{
				...
				if (m_nSubShowType==SUB_SHOW_TYPE_DISABLE)
